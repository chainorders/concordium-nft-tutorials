...
function PageButton(props: {
	page: IPage;
	isSelected: boolean;
	onClick: (path?: string, action?: ()=>void) => void;
}) {
	...
	return (
		<Button
			...
			key={page.name}
			onClick={() => onClick(page.href || page.path, page.action)}
			sx={{
				my: 2,
				color: "white",
				display: "block",
...
function Header(props: { pages: IPage[] }) {
	...
	const handleCloseNavMenu = (href?: string, action?: () => void) => {
		setAnchorElNav(null);
					...
					<Box sx={{ flexGrow: 1, display: { xs: "flex", md: "none" } }}>
						...
						>
							{props.pages.map((page) => (
								<MenuItem
									key={page.name}
									onClick={() => handleCloseNavMenu(page.href || page.path, page.action)}
						...
						{props.pages
							...
							.map((page) => (
								<PageButton
									...
									onClick={(path) => handleCloseNavMenu(path, page.action)}
								/>
							))}
					</Box>
					<Box
